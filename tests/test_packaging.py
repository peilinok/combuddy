...
def test_macos_dmg_script_customizes_finder_icon_layout():
    script = pathlib.Path("packaging/macos_dmg.sh").read_text(encoding="utf-8")
    assert "hdiutil attach" in script
    assert "osascript <<APPLESCRIPT" in script
    assert 'set dmgFolder to POSIX file "$MOUNT_DIR" as alias' in script
    assert "set current view of container window of dmgFolder to icon view" in script
    assert 'set position of item "combuddy.app"' in script
    assert 'set position of item "Applications"' in script
